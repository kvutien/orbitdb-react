/*
 * example of React application using orbitDB by calling the utilities in /src
 */
...
const Intro = () => (
    // displays the intro header of the HTML page
...
// object composed of an array of titles and an array of tags
const samples = {
...
// object composed of a title and an array of 3 tags
const newDocument = () => ({
    id: uuid(),
...
// returns an array of 1 id (the key) and 1 object composed of a title and an array of 3 tags
const newKeyValue = () => {
    const id = uuid();
    return [
        id,
...
const KeyValueDemo = () => {
    // create an Orbit database of type Key-value pairs
    const { db, records } = useOrbitDb(ORBIT_DB_KEYVALUE, {
        type: "keyvalue",
        create: true,
        public: true,
    });
    console.log("*** App.KeyValueDemo *** db", db, "records", records);
    const addKey = async () => {
